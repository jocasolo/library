...
public interface UserDAO extends CrudRepository<User, Integer> {
	...
	@Query(value = "SELECT new com.at.library.dto.UserDTO(u.id, u.dni, u.name, u.surname) from User as u WHERE (u.dni LIKE %:dni% OR :dni IS NULL) AND (u.name LIKE %:name% OR :name IS NULL) AND (u.surname LIKE %:surname% OR :surname IS NULL) AND status <> 'DELETED'")
