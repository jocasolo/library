...
public interface BookDAO extends CrudRepository<Book, Integer> {
	...
	@Query(value = "SELECT new com.at.library.dto.BookDTO(b.id, b.isbn, b.title, b.author) from Book as b WHERE (b.isbn LIKE %?1% OR ?1 IS NULL) AND (b.title LIKE %?2% OR ?2 IS NULL) AND (b.author LIKE %?3% OR ?3 IS NULL) AND status <> 'DELETED'")
